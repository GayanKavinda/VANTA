import os; os.chdir('C:/wamp64/apps/vanta')
t = []
def a(name, lines):
    t.append(f"\ndef {name}():")
    for l in lines:
...
# HTML Parser tests - Video
a("test_parser_video_src", [
...
a("test_parser_source_inside_video", [
    "from app.sources.html_parser import parse_html",
...
a("test_parser_source_inside_audio", [
    "from app.sources.html_parser import parse_html",
...
a("test_parser_source_inside_picture", [
    "from app.sources.html_parser import parse_html",
...
# HTML Parser tests - Image
a("test_parser_img_src", [
    "from app.sources.html_parser import parse_html",
...
a("test_srcset_width_descriptors", [
    "from app.sources.html_parser import _parse_srcset",
...
a("test_srcset_malformed_ignored", [
    "from app.sources.html_parser import _parse_srcset",
    "urls = _parse_srcset(', , invalid, ,')",
    "assert urls == []",
])

a("test_srcset_data_ignored", [
    "from app.sources.html_parser import _parse_srcset",
...
# Deduplication test
a("test_parser_deduplicates_same_url", [
    "from app.sources.html_parser import parse_html",
...
a("test_parser_links_still_discovered", [
    "from app.sources.html_parser import parse_html",
...
# Capability test
a("test_generic_adapter_has_media_discovery", [
...
a("test_generic_adapter_no_download", [
    "from app.sources.generic import GenericSourceAdapter",
    "from app.sources.capability import SourceCapability",
    "adapter = GenericSourceAdapter()",
...
result = "\n".join(t)
with open("tests/test_v200_phase32_generic_discovery.py", "w") as f:
    f.write(result)
print(f"Phase 3.2 tests written: {len(t)} tests")
